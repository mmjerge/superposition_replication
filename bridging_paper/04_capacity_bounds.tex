...
\end{equation}
Even if $|F_t|$ is large, the capacity bound (Theorem~\ref{thm:capacity}) applies with dimension $d$: superposition succeeds when $|F_t| \leq O(d^2 \cdot S^{(t)} / \epsilon)$. The model handles frontier growth by either (a) having sufficient dimension $d$, or (b) using iterative refinement (Section~\ref{sec:timespace}) to prune the frontier over time.
\end{remark}
