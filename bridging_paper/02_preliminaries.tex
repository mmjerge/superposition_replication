...
Let $[n] = \{1, \ldots, n\}$ denote the first $n$ positive integers. For vectors $\mathbf{u}, \mathbf{v} \in \R^m$, we write $\langle \mathbf{u}, \mathbf{v} \rangle = \mathbf{u}^\top \mathbf{v}$ for the inner product and $\|\mathbf{u}\| = \sqrt{\langle \mathbf{u}, \mathbf{u} \rangle}$ for the Euclidean norm. For a matrix $\mathbf{W} \in \R^{m \times n}$, we write $\mathbf{w}_i \in \R^m$ for its $i$-th column. The indicator function $\ind[\cdot]$ equals 1 if its argument is true and 0 otherwise.

\subsection{Representational Superposition}
\label{sec:rep_superposition}

Following \citet{elhage2022toy}, consider an autoencoder with encoder $\mathbf{W} \in \R^{m \times n}$ and tied decoder $\mathbf{W}^\top$, where $m < n$. The model receives sparse binary inputs $\mathbf{x} \in \{0, 1\}^n$ where each component $x_i$ activates independently with probability $p_i$. The reconstruction is:
\begin{equation}
